...
const getUpdatedClassNames = (className, value, breakpoint) => {
    return getUpdatedClassNameStyle(className, 'align', value, STYLE_VALUES, breakpoint);
}

function AlignContent({ blockProps: { setAttributes, attributes: { className } }, breakpoint }) {
    ...
    >
        ...
        >
            <Button isPressed={activeValue === 'full'} onClick={() => update('full')} label={__('Full width', 'responsive-block-controls')} icon={<Icon icon={stretchFullWidth} />} />
        </BaseControl>
    </ToolsPanelItem>
}
