...
const getActiveValue = (className, breakpoint) => {
    return getClassNameStyleValue(className, 'columns-direction', STYLE_VALUES, breakpoint);
}

const getUpdatedClassNames = (className, value, breakpoint) => {
    return getUpdatedClassNameStyle(className, 'columns-direction', value, STYLE_VALUES, breakpoint);
}

function ColumnsDirection({ blockProps: { setAttributes, attributes: { className } }, breakpoint }) {
