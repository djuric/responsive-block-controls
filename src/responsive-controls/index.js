...
function ResponsiveControls({ blockProps, breakpoint }) {
    ...
    const resetClassNames = () => {
        const updatedClassNames = CONTROLS_STYLES_SUPPORT.reduce((acc, styleName) => {
    ...
    return <ToolsPanel className="responsive-block-controls-tools-panel" label={`${breakpoint.title} screens`} panelId={breakpoint.name} resetAll={(callbacks) => {
        resetClassNames();

        // Only execute callbacks that are not empty functions.
        callbacks.filter((cb) => cb.toString() !== '() => {}').forEach(cb => cb());
    }}>
